<?php
	...
	$objconfig->table 	= "correlativo_patrimonio_estable";
	$objconfig->campoId	= "idcorrelativo";
       ...
       if($_POST["op"]==1)
        {   
           $objconfig->execute("insert into log(usuario,ip,descripcion,hora,tipo_examen,nombre_usuario,idusuario) 
                                values('".$_SESSION['nombre']."','".$ip."','SE REGISTRO NUEVO CORRELATIVO PARA EL ESTABLECIMIENTO: ".strtoupper($_POST["nombre_establecimiento"])."','".date("h:i:s A")."','UROCULTIVO','".$_POST["0form_nombre_usuario"]."','".$_POST["0form_idusuario"]."')");
        }else{
           $objconfig->execute("insert into log(usuario,ip,descripcion,hora,tipo_examen,nombre_usuario,idusuario) ) 
