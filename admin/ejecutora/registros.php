...
$sql = "SELECT idejecutora, descripcion, direccion,ruc, replegal,codejecutora, case when estareg=1 then 'ACTIVO' ELSE 'INACTIVO' end as estareg ";
$sql.= " FROM ejecutora WHERE 1=1";
$isFilterApply=0;
	...
	// if there is a search parameter, $requestData['search']['value'] contains search parameter
		...
		$sql.=" OR upper(codejecutora) LIKE upper ('%".$requestData['search']['value']."%') ";
		$sql.=" )";
...
foreach($query[1] as $items)
	...
	$nestedData[] = $items["idejecutora"];
	$nestedData[] = $items["descripcion"];
	$nestedData[] = $items["direccion"];
	$nestedData[] = $items["ruc"];
	$nestedData[] = $items["codejecutora"];
	$nestedData[] = $items["estareg"];
	$nestedData[] = "<button type='button'  data-target='#userModal' data-toggle='modal' data-backdrop='static' data-keyboard='false' onclick='cargar_form(2,".$items["idejecutora"].")' class='btn btn-block btn-info'>Editar</button>";
