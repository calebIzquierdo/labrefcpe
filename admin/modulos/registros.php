...
$sql = "SELECT idmodulo, descripcion, url, estareg ";
$sql.= " FROM modulos ";
...
$sql = "SELECT idmodulo, descripcion, url, (CASE WHEN estareg = 0 THEN 'INACTIVO' ELSE 'ACTIVO' END) AS  estado  ";
$sql.= " FROM modulos WHERE 1=1";
...
foreach($query[1] as $items)
	...
	$nestedData[] = strtoupper($items["url"]);
	$nestedData[] = $items["estado"];
	$nestedData[] = "<button type='button'  data-target='#userModal' data-toggle='modal' data-backdrop='static' data-keyboard='false' onclick='cargar_form(3,".$items["idmodulo"].")' class='btn btn-block btn-info'>Editar</button>";
