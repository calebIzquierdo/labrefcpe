...
$sql = "SELECT idred, ejecuta, direccion,replegal, estareg,descripcion,estareg ";

$sql.= " FROM vista_red WHERE 1=1";


	if( !empty($requestData['search']['value']) ) 
	{
	// if there is a search parameter, $requestData['search']['value'] contains search parameter
		$sql.=" AND ( UPPER(ejecuta) LIKE UPPER ('%".$requestData['search']['value']."%') ";    
...
foreach($query[1] as $items)
	...
	$nestedData[] = $items["idred"];
	$nestedData[] = $items["ejecuta"];
	$nestedData[] = $items["descripcion"];
	$nestedData[] = $items["direccion"];
	$nestedData[] = $items["replegal"];
	$nestedData[] = $items["estareg"];
	$nestedData[] = "<button type='button' data-target='#userModal' data-toggle='modal' data-backdrop='static' data-keyboard='false' onclick='cargar_form(2,".$items["idred"].")' class='btn btn-warning btn-primary btn-xs'>Editar</button>";
