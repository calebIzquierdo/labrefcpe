<?php
	...
	$objconfig->table 	= "personal";
	$objconfig->campoId	= "idpersonal";
     ...
     //   echo $sql;
        ...
        if($_POST["op"]==1)
        {
           $objconfig->execute("insert into log(usuario,ip,descripcion,hora) 
                                values('".$_SESSION['nombre']."','".$ip."','SE REALIZO LA INSERCION DEL NUEVO PERSONAL ".strtoupper($_POST["0form_nombres"])."','".date("h:i:s A")."')");
        }else{
           $objconfig->execute("insert into log(usuario,ip,descripcion,hora) 
                                values('".$_SESSION['nombre']."','".$ip."','SE REALIZO LA MODIFICACION DEL PERSONAL ".strtoupper($_POST["0form_nombres"])."','".date("h:i:s A")."')");
        }
?>