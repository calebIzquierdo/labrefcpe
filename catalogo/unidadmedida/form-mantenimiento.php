<?php
	...
	if($cod!=0)
	{
		$query = "select * from unidad_medida where idunidad=".$cod;
...
<form method="post" enctype="multipart/form-data"  id="user_form"  name="user_form">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">Mantimiento Unidad de Medida</h4>
				<input type="hidden" name="op" id="op" value="<?php echo $op; ?>"  />
			</div>
			<div class="modal-body">
				<label>Item</label>
				<input type="text" name="1form_idunidad" id="codigo" value="<?=$row[1]["idunidad"]?>" class="form-control"    />
				<br />
				<label>Descripcion</label>
				<input type="text" name="0form_descripcion" id="descripcion" value="<?=$row[1]["descripcion"]?>"  class="form-control" />
				<br />
				<label>Codigo</label>
				<input type="text" name="0form_ciiu" id="ciiu" value="<?=$row[1]["ciiu"]?>"  class="form-control" />
				<br />
