<?php
    ...
    $data= base64_decode($query1[1][0]['tramaxmlfirmado']);
...
try{
    ...
    /**Configure content */
    $mail->isHTML(true);
    $mail->Subject = utf8_decode($query1[1][0]['descripcion']).' '.$query1[1][0]['nrodocumento'];
    $body="";
    ...
    $body.='<ul style="color:gray;margin:0.3rem 0;">
        <li>'.$query1[1][0]['descripcion'].' '.$query1[1][0]['nrodocumento'].'</li>
        <li>Fecha de Emision: '.$query1[1][0]['fecharecepcion'].' '.$query1[1][0]['horareg'].'</li>
        <li>Total: S/.'.$query1[1][0]['monto'].'</li>
    </ul>';
    $body.='<hr style="color:gray; padding:0"/>';  

    $body.='<p style="font-size:.9rem; color:blue; padding: 0;margin:0">
    Tambien se adjunta en PDF el mismo documento, que puede ser 
    impresa y usada como una '.$query1[1][0]['descripcion'].' emitida de manera tradicional.</p>';
    '<p style="font-size:1.2rem; color:dimgray; font-weight:bold; padding: 0;margin:0">Saludos.</p>';
    $mail->Body=$body;

    $mail->AddAttachment($pdf_filename,$query1[1][0]['descripcion'].' '.$query1[1][0]['nrodocumento'].'.pdf');
    $mail->AddAttachment($xml_filename,$query1[1][0]['descripcion'].' '.$query1[1][0]['nrodocumento'].'.xml');
