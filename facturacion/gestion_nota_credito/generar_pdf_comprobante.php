	...
	class GenerarPDFComprobante extends PdfElectronico
	{
        function Header($item_pago){
            ...
            /* if(strlen($query[1]['descripcion'])>33){
                ...
                $this->setXY(5, $textypos);
                $this->Cell(0, 0, 'UNIDAD EJECUTORA HOSPITAL', 0, 1, "C");
                $textypos += 4;
                $this->setXY(5, $textypos);
                $this->Cell(0, 0, 'II-2 TARAPOTO', 0, 1, "C");
                $textypos += 4;
                $this->setXY(5, $textypos);
            ...
            $textypos += 4;
            $query = $this->execute_select("select P.idcomprobante, TC.descripcion, P.nrodocumento from pago P inner join tipo_comprobante TC on (P.idcomprobante= TC.idcomprobante) where P.idpago='".$item_pago."';");
            $this->setXY(5, $textypos);
            $this->SetFont('Courier', 'B', 10);    //Letra Arial, negrita (Bold), tam. 20
            $this->Cell(0, 0, utf8_decode("NOTA DE CREDITO"), 0, 1, "C");
            $textypos += 4;
            $this->setXY(5, $textypos);
        ...
        function contenido($item_pago){
            ...
            $query = $this->execute_select("select P.fecharecepcion, P.itempago, P.horareg, P.idingresomuestra, P.descuento, P.monto1, P.codbarra, P.monto, C.razonsocial, C.ruc, C.direccion from pago P inner join cliente C on (P.idcliente= C.idcliente) where P.idpago='".$item_pago."';");
