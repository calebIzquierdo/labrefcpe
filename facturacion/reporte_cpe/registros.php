<?php
	...
	$sql = "SELECT distinct P.itempago, P.fecharecepcion, A.descripcion as idarea, P.estareg, P.nrodocumento, TC.descripcion as idcomprobante, P.fechaemision, P.idcliente, P. monto AS total, TP.descripcion as idtipopago
	FROM pago P JOIN areas A ON (P.idarea=A.idarea) JOIN tipo_comprobante TC ON (P.idcomprobante=TC.idcomprobante) JOIN tipo_pago TP ON (P.idtipopago=TP.idtipopago) JOIN cliente C ON (P.idcliente=C.idcliente) where P.fechaemision between'".$_GET["fInicio"]."' and '".$_GET["fFin"]."'";
