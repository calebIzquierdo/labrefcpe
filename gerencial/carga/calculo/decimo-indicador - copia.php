<?php
	...
	for($i=0;$i<count($tipo);$i++)
	{
		if($tipo[$i]!=0)
		{
			...
			$sql3  = "select count(*) as cantidad from tablero_certificados where idindicador=10 and anio='".$anio."' and idcertificado=".$tipo[$i];
			$rows3 = $objconfig->execute_select($sql3);
			
			if($rows3[1]["cantidad"]==0)
			{
				$sql4 = "insert into tablero_certificados(idindicador,idcertificado,anio,".$meses[$mes].") values(10,".$tipo[$i].",'".$anio."',".$rows2[1]["valor"].")";
			}else{
				$sql4 = "update tablero_certificados set ".$meses[$mes]."=".$rows2[1]["valor"]." where idindicador=10 and anio='".$anio."' and idcertificado=".$tipo[$i];
			}
			//echo $sql4."<br>";
