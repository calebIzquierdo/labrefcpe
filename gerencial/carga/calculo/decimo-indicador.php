<?php
	...
	foreach($tDi[1] as $tpDiag)
	{
		...
		$sql2 = "select idespecialidad from referencia
				 ...
				 and idestado=2 and idanulado=0 and  idtiporeferencia=".$tpDiag["idtiporeferencia"]."
	...
	foreach($rows2[1] as $tr)
	{
			...
			{
				$sql1= "select count(iddiagnostico) as cantidad from tablero_diagnostico_envio where idindicador=10 and anio='".$anio."' 
				...
				$Cons = "select count(iddiagnostico) as valor from referencia_diagnostico where iddiagnostico=".$idDiag["iddiagnostico"]." 
						...
						where date_part('month',fecreferencia)=".$mes." and date_part('year',fecreferencia)='".$anio."' 
						and idestado=2 and idanulado=0 and idtiporeferencia=".$tpDiag["idtiporeferencia"]." and idespecialidad=".$tr["idespecialidad"]."
				...
				if ($tMes[1]["valor"]!=0 ){
					...
					{
						$sql55 = "insert into tablero_diagnostico_envio(idindicador,idestado,idtiporeferencia,
						idespecialidad,iddiagnostico,anio,".$meses[$mes].") 
						values(10,2,".$tpDiag["idtiporeferencia"].",".$tr["idespecialidad"].",".$idDiag["iddiagnostico"].",'".$anio."', ".$tMes[1]["valor"].")";
					} 
					else{
						$sql55 = "update tablero_diagnostico_envio set $meses[$mes]=".$tMes[1]["valor"]." 
						where idindicador=10 and anio='".$anio."' and iddiagnostico=".$idDiag["iddiagnostico"]." 
				...
				$sumAnio = "select sum(enero+febrero+marzo+abril+mayo+junio+julio+agosto+setiembre+octubre+noviembre+diciembre)::int as sumatotal 
							from tablero_diagnostico_envio where idindicador=10 and anio='".$anio."' and iddiagnostico=".$idDiag["iddiagnostico"]." 
							 and idtiporeferencia=".$tpDiag["idtiporeferencia"]." and idespecialidad=".$tr["idespecialidad"]." ";
		
				$TsumAnio = $objconfig->execute_select($sumAnio);
				$sqlSuma = "update tablero_diagnostico_envio set sumanio=".$TsumAnio[1]["sumatotal"]." 
				where idindicador=10 and anio='".$anio."' and iddiagnostico=".$idDiag["iddiagnostico"]." 
									 and idtiporeferencia=".$tpDiag["idtiporeferencia"]." and idespecialidad=".$tr["idespecialidad"];
		...
		$sql1  = "select count(*) as cantidad from tablero where idindicador=10 and anio='".$anio."'";
		$rows1 = $objconfig->execute_select($sql1);
	
		$sAnio = "select sum(".$meses[$mes].")::int as sumatotal 
		from tablero_diagnostico where idindicador=10 and anio='".$anio."' ";
		$TAnio = $objconfig->execute_select($sAnio);
				
		if($rows1[1]["cantidad"]==0)
		{
			
			$sql = "insert into tablero(idindicador,anio,".$meses[$mes].") values(10,'".$anio."',".$TAnio[1]["sumatotal"].")";
		} else{
			$sql = "update tablero set ".$meses[$mes]."=".$TAnio[1]["sumatotal"]." where idindicador=10 and anio='".$anio."'";
		}
	...
	$updInd = "update indicadores set valor_anterior=valor_actual,valor_actual=".$TAnio[1]["sumatotal"]." where idindicador=10";
	$objconfig->execute_select($updInd);
