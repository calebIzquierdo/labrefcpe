<?php
	$sql1 = "select date_part('year',c.fechareg),date_part('month',c.fechareg),certificados
			...
			group by date_part('year',c.fechareg),date_part('month',c.fechareg),certificados";
	//echo $sql1;
	$rows1 = $objconfig->execute_select($sql1);
	
	$tipo = explode("|",$rows1[1]["certificados"]);
	echo $rows1[1]["certificados"];
	for($i=0;$i<count($tipo);$i++)
	{
		if($tipo[$i]!=0)
		{
			$sql2 = "select date_part('year',c.fechareg),date_part('month',c.fechareg),count(*) as valor
					...
					inner join detmovimiento as d on(dd.itemdetmovimiento=d.item_detmovimiento)
					inner join cabmovimiento as c on(d.nromovimiento=c.nromovimiento and d.idtipomovimiento=c.idtipomovimiento)
					where c.idtipomovimiento=1 and date_part('year',c.fechareg)=".$anio." and date_part('month',c.fechareg)=".$mes." and certificados like '%".$tipo[$i]."%'
					group by date_part('year',c.fechareg),date_part('month',c.fechareg)";
			//echo $sql2."<br>";
			$rows2 = $objconfig->execute_select($sql2);
			
			$sql3  = "select count(*) as cantidad from tablero_certificados where idindicador=9 and anio='".$anio."' and idcertificado=".$tipo[$i];
			$rows3 = $objconfig->execute_select($sql3);
			
			if($rows3[1]["cantidad"]==0)
			{
				$sql4 = "insert into tablero_certificados(idindicador,idcertificado,anio,".$meses[$mes].") values(9,".$tipo[$i].",'".$anio."',".$rows2[1]["valor"].")";
			}else{
				$sql4 = "update tablero_certificados set ".$meses[$mes]."=".$rows2[1]["valor"]." where idindicador=9 and anio='".$anio."' and idcertificado=".$tipo[$i];
			}
			//echo $sql3."<br>";
