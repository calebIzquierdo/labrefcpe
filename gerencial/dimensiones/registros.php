...
//si estas en la primera pagin ale asignas los valores iniciales
	...
	if($valor!="")
	{
		$where = " where upper(descripcion) ilike '%".$valor."%'";
	}
	
	$objconfig->table 	= "dimensiones";
	$objconfig->campoId = "iddimension";
	...
	$queryP = $objconfig->genera_sql($where,$objconfig->campoId,""," limit $TAMANO_PAGINA offset $inicio");
	//echo $queryP;
	...
	foreach($rowP[1] as $items)
	{
		...
		echo "<td align='center' width='20%'>".$items["iddimension"]."</td>";
		echo "<td width='60%'>".strtoupper($items["descripcion"])."</td>";
		echo "<td align='center' width='10%'>".$estareg."</td>";
		echo "<td width='10%' align='center'><img src='../../img/edit.png' style='cursor:pointer;' title='Editar Registro' onclick='cargar_form(2,".$items["iddimension"].")' /></td>";
		echo "</tr>";
	}
	
	echo "<tr>";
    echo "<td colspan='4' class='PieIndex'>";
