<?php
	...
	*/
    ...
    if($_POST["op"]==1)
    {
		...
		*/
		//$nrorequerimiento = substr(str_repeat(0, 7).$correlativo, - 7);
		$nrorequerimiento = substr(str_repeat(0, 7).$next, - 7);
		
    }else{
		
        $next	= $_POST["1form_idrequerimiento"];
		$nrorequerimiento = substr(str_repeat(0, 7).$next, - 7);
    }
   ...
   // echo $query ;
	...
	$sql1 = "update requerimiento set correlativo='".$nrorequerimiento."' where idrequerimiento=".$next;
	$objconfig->execute($sql1);
	...
	$objconfig->execute($del1);
		...
		{	
			...
			{
				$sql1 = "insert into requerimiento_detalle(idrequerimiento,idmaterial,cantidad,especificaciones,idmarca)
					values(".$next.",".$_POST["idmaterial".$i].",".$_POST["cantidad".$i].",'".$_POST["especificaciones".$i]."','1')";
					echo $sql1 ;
				$objconfig->execute($sql1);
			}
		}
