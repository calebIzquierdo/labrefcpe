...
function buscar_stock(idmat)
{
	objindex="Stock"
    var ventana=window.open("../lista/stock/index.php?idm="+idmat+"", 'Stock', 'width=980,height=600,resizable=no, scrollbars=yes, status=yes,location=yes');
    ventana.focus();
}
...
function validaStock(id) {
	var capr = parseFloat($("#cantidad"+id).val())
	var apro = parseFloat($("#cantidad_a"+id).val())
	var stoc = parseFloat($("#stock"+id).val())
	
	if (apro > capr || apro > stoc ){
		alert("El monto aprobado no debe ser mayor que Monto Requerido y/o Stock Actual")
		//alert("El monto aprobado no debe ser mayor que el monto Requerido")
		//$("#cantidad_a"+id).focus()
		//$("#cantidad_a").focus(); 
	}
	/*
	if (apro > stoc ){
		alert("El monto aprobado no debe ser mayor que Stock Actual")
		//$("#cantidad_a"+id).focus()
		//$("#cantidad_a").focus(); 
	}
	*/
