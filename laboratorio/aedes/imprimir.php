<?php 
	...
	class impresion extends clsreporte
	{
		 function Header(){
			...
			$this->Ln(8);
				...
				$rp  = $this->execute_select("select nroreporte,codbarra from vista_aedes where idaedes=".$id);
				
				$nombreDoc=$rp[1]["codbarra"];
		...
		function contenido($id)
		{
			...
			$queryG = "select iddepartamento, idprovincia, iddistrito,  departamento, provincia, distrito 
						from vista_aedes  where estareg=1 and idaedes=".$id." 
						group by iddepartamento, idprovincia, iddistrito,departamento, provincia, distrito";
		...
		//	echo $queryG;
			...
			*/
                $queryW = "select * from vista_aedes_consolidado where idaedes =".$id." order by idzona asc ";
			
                $rowsD = $this->execute_select($queryW,1);
			
				foreach($rowsD[1] as $rows){
					$n++;
						$c1p = $this->execute_select("select count(idaedes_a) as c1positivo from aedes_muestra where idaedes=".$id." and idfoco=1 and idzona=".$rows["idzona"]." and idaedes_a>0" );
						$c2p = $this->execute_select("select count(idaedes_a) as c2positivo from aedes_muestra where idaedes=".$id." and idfoco=2 and idzona=".$rows["idzona"]." and idaedes_a>0" );
						$c3p = $this->execute_select("select count(idaedes_a) as c3positivo from aedes_muestra where idaedes=".$id." and idfoco=3 and idzona=".$rows["idzona"]." and idaedes_a>0" );
						$c4p = $this->execute_select("select count(idaedes_a) as c4positivo from aedes_muestra where idaedes=".$id." and idfoco=4 and idzona=".$rows["idzona"]." and idaedes_a>0" );
						$c5p = $this->execute_select("select count(idaedes_a) as c5positivo from aedes_muestra where idaedes=".$id." and idfoco=5 and idzona=".$rows["idzona"]." and idaedes_a>0" );
						$c6p = $this->execute_select("select count(idaedes_a) as c6positivo from aedes_muestra where idaedes=".$id." and idfoco=6 and idzona=".$rows["idzona"]." and idaedes_a>0" );
						$c7p = $this->execute_select("select count(idaedes_a) as c7positivo from aedes_muestra where idaedes=".$id." and idfoco=7 and idzona=".$rows["idzona"]." and idaedes_a>0" );
						$c8p = $this->execute_select("select count(idaedes_a) as c8positivo from aedes_muestra where idaedes=".$id." and idfoco=8 and idzona=".$rows["idzona"]." and idaedes_a>0" );
						...
						$this->Cell(20,5,substr(strtoupper(utf8_decode($rows["localidad"])),0,14),1,0,'L');
		...
		//	}
			...
			$queryDGs = "select iddepartamento, idprovincia, iddistrito,  departamento, provincia, distrito 
						from vista_aedes  where estareg=1 and idaedes=".$id." 
						group by iddepartamento, idprovincia, iddistrito,departamento, provincia, distrito";

						
		//	echo $queryG;
			$rowsG2 = $this->execute_select($queryDGs);
			...
			$rp  = $this->execute_select("select idestadomuestra,observacion from aedes where idaedes=".$id);
			...
			if ($rp[1]["idestadomuestra"]==2){
				$this->RotatedImage('../../img/observacion.png',25,90,250,30,15);
				// $this->RotatedImage('circle.png',85,60,40,16,45);
				// $this->RotatedText(100,60,'Hello!',45);
				// $this->Image("../../img/observacion.png",85,60,140,56,'PNG',"http://hospitaltarapoto.gob.pe/");
			}
	...
	$pdf->Output('Consolidado_'.$nombreDoc.'.pdf', 'I');
