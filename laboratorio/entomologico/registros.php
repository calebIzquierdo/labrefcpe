...
*/
	...
	$sql = " select identomologia, fecharecepcion, estab_solicita,codbarra, zona, local, vprogram, vinspec, vpositiva, rpositiva,rinspeccionado, mrecibida,  tp_interv,estareg,redes ,micro_red";
	...
	if( !empty($requestData['search']['value']) ) 
	{
		...
		$sql.=" OR upper (zona) LIKE upper ('%".$requestData['search']['value']."%' )";
	...
	foreach($query[1] as $items)
		...
		$nestedData[] = $items["zona"];
		$nestedData[] = $items["local"];
		$nestedData[] = $items["vinspec"];
		$nestedData[] = $items["mrecibida"];
		$nestedData[] = $items["rinspeccionado"];
		$nestedData[] = $items["rpositiva"];
		$nestedData[] = $items["rpositiva"];
	...
	//	$nestedData[] = "<button class='btn btn-success center-block' data-toggle='modal' data-backdrop='static' data-keyboard='false' data-target='#impresiones' onclick='imprimir(".$items["identomologia"].")'><span class='glyphicon glyphicon-print'></span> Imprimir</button>";
		//$nestedData[] = "";
		$nestedData[] = "";
		$nestedData[] = "";
