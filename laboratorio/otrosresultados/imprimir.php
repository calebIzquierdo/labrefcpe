	...
	class impresion extends clsreporte
	{
		...
		*/
		function contenido($nromovimiento)
		{
			...
			$query = $this->execute_select("SELECT idotrosresultados, razonsocial, ruc, enfermedad, edad, medico, fechareg, establecimiento, observaciones
										FROM view_otrosresultados 
										WHERE idotrosresultados = ".$nromovimiento);
					
			$this->Ln(5);            
			$this->Cell(30,5,"Nombre Paciente ",0,0,'L');
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(100,5,strtoupper(utf8_decode($query[1]["razonsocial"])),0,0,'L');
		
			$this->Cell(27,5,utf8_decode("N°. D.N.I"),0,0,'L');
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(10,5,$query[1]["ruc"],0,1,'L');
						
			$this->Cell(30,5,utf8_decode("Examen Solicitado"),0,0,'L');
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(100,5,strtoupper(utf8_decode($query[1]["enfermedad"])),0,0,'L');
			
			$this->Cell(27,5,utf8_decode("F. Nacimiento"),0,0,'L');
			$this->Cell(5,5,":",0,0,'C');
			// $this->Cell(30,5,$this->FechaDMY2($query[1]["fnacimiento"]),0,1,'L');
			...
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(10,5,utf8_decode($query[1]["edad"]." años"),0,1,'L');
			
			$this->Cell(30,5,utf8_decode("Medico Solicitante"),0,0,'L');
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(100,5,strtoupper(utf8_decode($query[1]["medico"])),0,0,'L');
						
			$this->Cell(27,5,utf8_decode("F. Toma Muestra"),0,0,'L');
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(10,5,$this->FechaDMY2($query[1]["fechareg"]),0,1,'L');
			
			
			
			$this->Cell(30,5,utf8_decode("EE.SS. Solicitante"),0,0,'L');
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(100,5,strtoupper(utf8_decode($query[1]["establecimiento"])),0,0,'L');
			...
			$prueb = $this->execute_select("SELECT fecharegistro, descripcion, valores,idtipo_examen, resultado
							FROM view_otrosresultadosdet
							WHERE idotrosresultados = ".$query[1]["idotrosresultados"],1);
			...
			foreach($prueb[1] as $prue){
				...
				$this->Row(array($count,$prue["fecharegistro"], $prue["descripcion"], $prue["valores"], utf8_decode($prue["resultado"])),0);
			}
			
			$this->Ln(10);
			$prueb2 = $this->execute_select("SELECT fecharegistro, descripcion, valores,idtipo_examen, resultado
							FROM view_otrosresultadosdet
							WHERE idotrosresultados = ".$query[1]["idotrosresultados"]);
			if ($prueb2[1]["idtipo_examen"]!=82){
		...
		function Footer(){
			...
			$this->Image("img/codigoqr.png",10,260,25,25);  
			
			$this->Image("../../img/firmaHeriberto.jpg",130,255,80,35,'JPG',"http://hospitaltarapoto.gob.pe/");         
            $this->SetY(-10);
