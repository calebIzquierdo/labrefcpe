	...
	class impresion extends clsreporte
	{
		...
		*/
		function contenido($nromovimiento)
		{
			...
			$query = $this->execute_select("select  idpsa,pacientes, edadactual,medico_solicita,idpaciente,idestablesolicita,
										tipmuestra,fechatoma,idingresomuestra,observaciones,codbarra,medico_solicita, fecharesultado,
										hora,valorresultado,idatendido
										FROM vista_psa 
										where idpsa=".$nromovimiento);
			...
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(100,5,strtoupper(utf8_decode($query[1]["medico_solicita"])),0,0,'L');
			...
			$this->Ln(10);
			$this->SetFont('Arial','B',12);
            $this->Cell(10,5,"RESULTADO DE PRUEBA REALIZADA",0,0,'L');
            $this->Ln(10);
			$this->SetFont('Arial','B',10);
		    $this->SetWidths(array(10,20,20,25,15,20,20,40,25));
			$this->SetAligns(array('C','C','C','C','C','C','C','C'));
			$this->Row(array("#","Fecha","Hora","Prueba","Valor","Unidad","Ran. Ref.","Metodo","Cod. Muestra"));
			$this->SetFont('Arial','',10);
			$this->Row(array("1",$this->FechaDMY2($query[1]["fecharesultado"]),$query[1]["hora"],"PSA TOTAL",$query[1]["valorresultado"],"ng/ml", "<= 4.03","Quimioluminiscencia",$query[1]["codbarra"])); 
			//echo $query;
  ...
  /*
			$prueb = $this->execute_select("select  t.fecharesultado,tp.descripcion as tiprueba ,t.valor, 
						tt.descripcion as titorch , tp.referencia
							...
							where idpsa=".$query[1]["idpsa"],1);
			$this->SetFont('Arial','',10);
			foreach($prueb[1] as $prue){
				$count++;
				$this->SetWidths(array(10,30,50,50,50));
				$this->SetAligns(array('C','C','L','C','C'));
				$this->Row(array($count,$this->FechaDMY2($query[1]["fecharesultado"]),$prue["tiprueba"],$prue["valor"],utf8_decode($prue["titorch"])),0);
				}
			*/
			$this->Ln(10);
		...
		*/
		...
		//	$this->Ln(5);
			...
			$realiza = $this->execute_select("select  idpersonal, nombres, apellidos, nrodocumento, nrodocumento ||' - ' || apellidos || ', ' || nombres as realizadopor from vista_personal where idpersonal=".$query[1]["idatendido"]);
			$this->Cell(30,5,utf8_decode("Realizado Por:"),0,0,'L');
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(100,5,strtoupper(utf8_decode($realiza[1]["realizadopor"])),0,0,'L');
