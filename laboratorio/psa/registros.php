<?php
   ...
   $idusuario = explode("|",$_SESSION['nombre']);
	...
	switch ($est[1]["idnivelreporte"]) 
	{
		case 1:
		   if($est[1]["idestablecimiento"]==388 || $est[1]["idestablecimiento"]==402 ){
				$where =" WHERE idestablesolicita=388 or idestablesolicita=402";
			}else {
				$where =" WHERE idestablesolicita IN(select idestablecimiento 
	...
	$consul = "select idpsa from vista_psa";
	$query1=$objconfig->execute_select($consul,1) ;


	$sql = " select idpsa, fecharecepcion, pacientes,edadactual,tipmuestra, nrodocumento, nrorecibo,estab_solicita, 
	...
	foreach($query[1] as $items)
		...
		if ($items["estareg"]==1 && $est[1]["editar"]==1){
		$nestedData[] = "<button type='button'  data-target='#userModal' data-toggle='modal' data-backdrop='static' data-keyboard='false' onclick='cargar_resltados(2,".$items["idpsa"].")' class='btn btn-outline btn-info btn-primary btn-xs'>Resultado</button>";
		$nestedData[] = "<button class='btn btn-success center-block' data-toggle='modal' data-backdrop='static' data-keyboard='false' data-target='#impresiones' onclick='imprimir(".$items["idpsa"].")'><span class='glyphicon glyphicon-print'></span> Imprimir</button>";
		$nestedData[] = "<button type='button'  onclick='anular_referencia(".$items["idpsa"].",".$items["idpsa"].")' class='btn btn-outline btn-danger btn-xs'>Elimar Registro</button>";
		} else if (($items["estareg"]==2 && $est[1]["editar"]==1)){
