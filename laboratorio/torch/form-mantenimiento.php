<?php
	...
	if($cod!=0)
	{
		$query = "select * from torch where idtorch=".$cod;
...
-->
				...
				<input type="hidden" name="idtipo_examen" id="idtipo_examen" value="70"  />
				
				<h3 class="modal-title text-center">Registro de Examenes Torch</h3>
			</div>
            ...
            <!--    <div class="container">  -->
                   ...
                   <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                    ...
                    <div class="tab-content" id="pills-tabContent" >
                        <div class="tab-pane fade active in" id="home">
							...
							<div class="row">
							<div class="col-md-12">
							<input type="hidden" name="1form_idtorch" id="codigo" value="<?=$row[1]["idtorch"]?>"   <?php echo $readonly; ?>   class="form-control"/>
								...
								<div class="col-md-1">
									...
									<div class="col-md-2">
										<div class="input-group date form_date" data-date="" data-date-format="dd/mm/yyyy"  data-link-field="dtp_input2" data-link-format="yyyy-mm-dd">
											<input class="form-control" type="text" name="fecharesultado" id="fecharesultado" value=""   >
											<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
									   
										</div>
									</div>
									<div class="col-md-4" id="div-tipprueba">
                                       
                                    </div>
									<div class="col-md-2">
                                        <input type="text" name="valor" id="valor" value=""  class="form-control" placeholder="Valor" data-toggle="tooltip" data-placement="top" title="Valor de Prueba" />
                                    </div>
                                   
                                    <div class="col-md-2">
                                        <select id="tipotorch" name="tipotorch" class="form-control" placeholder="Resultado" data-toggle="tooltip" data-placement="top" title="Resultado"  >
                                            <option value="0"></option>
                                            <?php
                                            $queryT = "select idtipotorch, descripcion from tipo_torch where estareg=1 order by descripcion asc";
                                            $itemsT = $objconfig->execute_select($queryT,1);

                                            foreach($itemsT[1] as $rowT)
                                            {
                                                $selected="";
                                                if($rowT["idtipotorch"]==$row[1]["idtipotorch"]){$selected="selected='selected'";}
                                                echo "<option value='".$rowT["idtipotorch"]."' ".$selected." >".strtoupper($rowT["descripcion"])."</option>";
                                            }
                                            ?>
                                        </select>
				...
				<div class="panel panel-info ">
					...
					<div class="panel-heading">LISTADO DE Pruebas Realizados </div>
					...
					<div class="panel-body">
						<table class="table table-striped table-bordered table-hover table-responsive" id="tbprueba" name="tbprueba" >
							...
							$sqlT = "select m.idtorch, m.fecharesultado, m.idtipoprueba,m.idtipotorch, e.descripcion as tipprueba, m.valor, 
									o.descripcion as tiptor, m.idingresomuestra
									
									from torch_prueba as m
									inner join tipo_prueba as e on(e.idtipoprueba=m.idtipoprueba)
									inner join tipo_torch as o on(o.idtipotorch=m.idtipotorch)
									where m.idtorch=".$cod." ";
							...
							foreach($rowTr[1] as $rRt)
							{
								...
								<tr id='itemprueba<?php echo $count_prue; ?>' name='itemprueba<?php echo $count_prue; ?>' >
									<td >
										<input type='hidden' name='idingresomuestra<?php echo $count_prue; ?>' id='idingresomuestra<?php echo $count_prue; ?>' value='<?php echo $rRt["idingresomuestra"]; ?>' />
										<?php echo $count_prue ; ?>
									</td>
									<td>
										<input type='hidden' name='fecharesultado<?php echo $count_prue; ?>' id='fecharesultado<?php echo $count_prue; ?>' value='<?php echo $rRt["fecharesultado"]; ?>' />
										<?php echo strtoupper($rRt["fecharesultado"] ); ?>
									</td>
									<td>
										<input type='hidden' name='idtipoprueba<?php echo $count_prue; ?>' id='idtipoprueba<?php echo $count_prue; ?>' value='<?php echo $rRt["idtipoprueba"]; ?>' />
									...
									<td>
									<input type='hidden' name='idtipotorch<?php echo $count_prue; ?>' id='idtipotorch<?php echo $count_prue; ?>' value='<?php echo $rRt["idtipotorch"]; ?>' />
										<?php echo strtoupper($rRt["tiptor"] ); ?>
									</td>
									<td >
...
<script type="text/javascript">
	...
	tipo_prueba(<?php echo $row[1]["idingresomuestra"];?>)
