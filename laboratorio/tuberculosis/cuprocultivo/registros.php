...
*/
	...
	$sql = "SELECT idcoprocultivo  ";
	$sql.= " FROM vista_coprocultivo ";
	...
	$consul = "select idcoprocultivo from vista_coprocultivo";
...
*/
	...
	$sql = " select idcoprocultivo, fecharecepcion, pacientes,edadactual,tipmuestra,  nrodocumento, nrorecibo,tipseguro, codbarra,
			tipmuestra, medico_solicita, estab_solicita,nombre_usuario,fechadigitacion,estareg,estado_examen ";
	$sql.= " FROM vista_coprocultivo WHERE 1=1 ";
	...
	foreach($query[1] as $items)
		...
		//$nestedData[] = $n;
		$nestedData[] = $items["idcoprocultivo"];
		...
		if ($items["estareg"]==1){
		$nestedData[] = "<button type='button'  data-target='#userModal' data-toggle='modal' data-backdrop='static' data-keyboard='false' onclick='cargar_form(2,".$items["idcoprocultivo"].")' class='btn btn-outline btn-warning btn-primary btn-xs'>Editar</button>";
		$nestedData[] = "<button class='btn btn-success center-block' data-toggle='modal' data-backdrop='static' data-keyboard='false' data-target='#impresiones' onclick='imprimir(".$items["idcoprocultivo"].")'><span class='glyphicon glyphicon-print'></span> Imprimir</button>";
		//$nestedData[] = "<button type='button'  data-target='#userModal' data-toggle='modal' data-backdrop='static' data-keyboard='false' onclick='cargar_form(3,".$items["idcoprocultivo"].")' class='btn btn-outline btn-info btn-info btn-xs'>Resultado</button>";
		$nestedData[] = "<button type='button'  onclick='anular_referencia(".$items["idcoprocultivo"].",".$items["idcoprocultivo"].")' class='btn btn-outline btn-danger btn-xs'>Elimar Registro</button>";
