...
*/
	...
	$sql = "SELECT iddxhongo  ";
	$sql.= " FROM vista_dxhongo ";
	...
	$consul = "select iddxhongo from vista_dxhongo";
...
*/
	...
	$sql = " select iddxhongo, fecharecepcion, pacientes,edadactual, nrodocumento, nrorecibo,tipseguro, 
			tipmuestra, medico_solicita, estab_solicita,nombre_usuario,fechadigitacion,estareg,estado_examen ";
	$sql.= " FROM vista_dxhongo WHERE 1=1 ";
	...
	if( !empty($requestData['search']['value']) ) 
	{
		...
		$sql.=" OR upper (tipseguro) LIKE upper ('%".$requestData['search']['value']."%' )";
	...
	foreach($query[1] as $items)
		...
		$nestedData[] = $items["nrorecibo"];
		$nestedData[] = $items["tipseguro"];
		...
		if ($items["estareg"]==1){
		$nestedData[] = "<button type='button'  data-target='#userModal' data-toggle='modal' data-backdrop='static' data-keyboard='false' onclick='cargar_form(2,".$items["iddxhongo"].")' class='btn btn-outline btn-warning btn-primary btn-xs'>Editar</button>";
		$nestedData[] = "<button type='button'  data-target='#userModal' data-toggle='modal' data-backdrop='static' data-keyboard='false' onclick='cargar_form(3,".$items["iddxhongo"].")' class='btn btn-outline btn-info btn-info btn-xs'>Resultado</button>";
		$nestedData[] = "<button type='button'  onclick='anular_referencia(".$items["iddxhongo"].",".$items["iddxhongo"].")' class='btn btn-outline btn-danger btn-xs'>Elimar Registro</button>";
		} else if (($items["estareg"]==2)){
			$nestedData[] = "PROCESADO";
			//$nestedData[] = "<button type='button'  data-target='#userModal' data-toggle='modal' data-backdrop='static' data-keyboard='false' onclick='cargar_form(2,".$items["iddxhongo"].")' class='btn btn-outline btn-warning btn-primary btn-xs'>Editar</button>";
			$nestedData[] = "<button class='btn btn-success center-block' data-toggle='modal' data-backdrop='static' data-keyboard='false' data-target='#impresiones' onclick='imprimir(".$items["iddxhongo"].")'><span class='glyphicon glyphicon-print'></span> Imprimir</button>";
			$nestedData[] = "PROCESADO";
		}else {
			$nestedData[] = "ANULADO";
			$nestedData[] = "ANULADO" ;
