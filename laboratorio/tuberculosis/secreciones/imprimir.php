<?php 
	...
	class impresion extends clsreporte
	{
		 function Header(){
			...
			$tit2 	= "DIAGNOSTICO BACTERIOLÓGICO DE SECRECIONES";
		...
		function contenido($nromovimiento)
		{
			...
			$query = $this->execute_select("select  Idsecreciones,idtipourocultivo, fecharecepcion, codbarra, edadactual, fechatoma, idtipomuestra,
										fecharesultado, observaciones, coddiagnostico, idtipo, fechadigitacion, pacientes, nrodocumento,recuento,
										medico_solicita, tipmuestra, tipseguro,estado_examen,idatendido,nroingreso,idetiologia,gram,ziehl,
										exfisico, interpositivo,internegativo
										FROM vista_secreciones 
										where idsecreciones=".$nromovimiento);
			...
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(70,5,strtoupper(utf8_decode("SECRECIONES")),0,1,'L');
			...
			$tipUrocul = $this->execute_select("select  descripcion from tipo_urocultivo where idtipourocultivo=".$query[1]["idtipourocultivo"]);
			if ($query[1]["idtipourocultivo"]==1){
			...
			$this->Cell(5,5,":",0,0,'C'); 
			$this->Cell(70,5,utf8_decode($query[1]["internegativo"]),0,1,'L');
			$this->Ln(2);
			$this->Cell(50,5,utf8_decode("POSITIVO"),0,0,'L');
			$this->Cell(5,5,":",0,0,'C');
			$this->Cell(70,5,utf8_decode($query[1]["interpositivo"]),0,1,'L');
			$this->Ln(5);
			$diag = $this->execute_select("select  descripcion from tipo_etiologia where idetiologia=".$query[1]["idetiologia"]);
			
			if ($query[1]["idtipourocultivo"]==2)
			{
				$h	=	2;
				$X	=	65; // POSICIÓN DE HORIZONTAL
				$Y	=	145;	// POSICIÓN VERTICAL
				$alto	=	5;
				$ancho	=	140;
				$borde	=	0;
				$alineacion2	=	'J';
				...
				$this->Cell(5,5,":",0,0,'C');
				$this->Cell(70,5,utf8_decode($diag[1]["descripcion"]),0,1,'L');
				$this->Cell(50,5,utf8_decode("Gram"),0,0,'L');
				$this->Cell(5,5,":",0,0,'C');
				$this->Cell(70,5,$query[1]["gram"]." ",0,1,'L');
				$this->Cell(50,5,utf8_decode("Ziehl Neelsen"),0,0,'L');
				$this->Cell(5,5,":",0,0,'C');
				$this->SetFont('Arial','',8);
				$this->SetXY($X,$Y);
				$this->MultiCell($ancho,$alto,utf8_decode($query[1]["ziehl"]),$borde, $alineacion2);
				$this->SetFont('Arial','',10);
				$this->Cell(50,5,utf8_decode("Examén Directo"),0,0,'L');				
				$this->Cell(5,5,":",0,0,'C');
				$this->Cell(120,5,$query[1]["exfisico"]." ",0,1,'L');
				
				$this->Cell(50,5,utf8_decode("OBSERVACIONES"),0,0,'L');
				$this->Cell(5,5,":",0,0,'C');
				$this->MultiCell($ancho,$alto,utf8_decode($query[1]["observaciones"]),$borde, $alineacion2);
				...
				$Y	=	183;	// POSICIÓN VERTICAL
				$alto	=	5;
				$ancho	=	65;
				$borde	=	1;
				$alineacion2	=	'C';
				
				if(strlen($query[1]["ziehl"])> 30){
				$this->Ln(2);	
				}else{
					$this->Ln(7);
				}
				...
				$this->SetFont('Arial','',8);
				$count1=0;
				$queryD = "select u.idantibiograma, a.descripcion from secreciones_antibiograma as u
						  inner join antibiograma as a on(u.idantibiograma=a.idantibiograma)
						  where u.idsecreciones=".$nromovimiento." and u.idtipoantibiograma=1";
				...
				$count2=0;
				$query2 = "select u.idantibiograma, a.descripcion from secreciones_antibiograma as u
						  inner join antibiograma as a on(u.idantibiograma=a.idantibiograma)
						  where u.idsecreciones=".$nromovimiento." and u.idtipoantibiograma=2";
				...
				$count3=0;
				$query3 = "select u.idantibiograma, a.descripcion from secreciones_antibiograma as u
						  inner join antibiograma as a on(u.idantibiograma=a.idantibiograma)
						  where u.idsecreciones=".$nromovimiento." and u.idtipoantibiograma=3";
