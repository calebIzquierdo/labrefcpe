    ...
    <div class="modal-content modal-lg">
		...
		<div class="row col-md-12">
			...
			<?php
				...
				if($row[1]["img_paciente"]!=""){
					echo "<img src='data:image/png;base64,". $row[1]["img_paciente"]."' width=50% height:300px>";
				} else {
					//echo '<script> buscar_dni() </script>';
					?>
					<script> buscar_dni() </script>
					<img id="imgpaciente" name="imgpaciente" style="background:blue; width:50%; height:300px; max-height:350px; object-fit:cover;object-position: center center;" src="https://static.vecteezy.com/system/resources/previews/005/228/939/original/avatar-man-face-silhouette-user-sign-person-profile-picture-male-icon-black-color-illustration-flat-style-image-vector.jpg" />
					<?php
			...
			?>
			<input type="hidden" name="0form_img_paciente" id="img_paciente" value="<?=$row[1]["img_paciente"];?>"  />
		...
		<!--		<input type="hidden" name="user_id" id="user_id" />
				...
				<button type="button" class="btn btn-default" onclick="recarga2(carpeta)" data-dismiss="modal">Cerrar</button>
            </div>
		</div>
	</form>
