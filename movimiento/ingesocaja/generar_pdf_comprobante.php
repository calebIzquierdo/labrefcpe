<?php
	...
	class GenerarPDFComprobante extends PdfElectronico
	{
        ...
        function contenido($item_pago){
            ...
            $query = $this->execute_select("select P.fecharecepcion,fechaemision, P.itempago, P.horareg, P.idingresomuestra, P.descuento, P.monto1, P.codbarra, P.monto, C.razonsocial, C.ruc, C.direccion from pago P inner join cliente C on (P.idcliente= C.idcliente) where P.itempago='".$item_pago."';");
            
            $this->SetXY(2, $textypos);
            $this->SetFont('Courier', 'B', 8);    //Letra Arial, negrita (Bold), tam. 20
            $this->Cell(0, 0, utf8_decode("FECHA EMISION: ") . date('d/m/Y', strtotime($query[1]['fechaemision'])) . str_pad("HORA: " . date("H:i:s", strtotime($query[1]['horareg'])), 17, ' ', STR_PAD_LEFT));
            $textypos += 4;
            $this->SetXY(2, $textypos);
