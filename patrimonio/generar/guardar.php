<?php
	...
	$objconfig->table 	= "codigopatrimonio_estable";
	$objconfig->campoId	= "idcodigopatrimonio";
	...
	/*$del1 = "delete from codigopatrimonio_detalle where idingresomuestra=".$next;
	$objconfig->execute($del1);*/
	
		for($i=1;$i<=$_POST["0form_cantidad"];$i++)
		{
			$sql1 = "insert into codigopatrimonio_detalle(idcodigopatrimonio,codigopatrimonio )
                values(".$next.",'".$_POST["codigopatrimonio".$i]."')";
		echo $sql1."\n ";
		$objconfig->execute($sql1);
		}
	$ncorrela = $_POST["cant"]+1;
	 $objconfig->execute("update correlativo_patrimonio_estable set correlativo='".$ncorrela."' where idcorrelativo=".$_POST["0form_item"]);
