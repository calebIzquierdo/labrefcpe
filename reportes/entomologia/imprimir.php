            ...
            function contenido($fechainicio,$fechafinal,$codred,$codmicrored,$codestablecimiento,$codpar)
                    {
                            ...
                            $queryG = "select f.iddepartamento, f.idprovincia, f.iddistrito, f.localidad as local from consolidado as f
                            ...
                            $rowsG = $this->execute_select($queryG);
                                    ...
                                    $this->cargar_ficha_entomologica($fechainicio,$fechafinal,$rowsG[1]["iddepartamento"],$rowsG[1]["idprovincia"],$rowsG[1]["iddistrito"],$rowsG[1]["local"],$and);
                ...
                $this->SetX($x);
                            ...
                            $queryD = "select d.descripcion as departamento, p.descripcion as provincia, ds.descripcion as distrito, 
                                                    cm.localidad as local, sum(cm.c1positivo) as tanque, sum(cm.c2positivo) as barril, sum(cm.c3positivo) as baldes, 
                                                    sum(cm.c4positivo) as ollas,sum(cm.c5positivo) as floreros, sum(cm.c6positivo) as llantas, 
                                                    sum(cm.c7positivo) as inservibles, sum(cm.c8positivo) as orecipientes, sum(cm.c1) as c1, 
                                                    sum(cm.c2) as c2, sum(cm.c3) as c3, sum(cm.c4) as c4, sum(cm.c5) as c5, sum(cm.c6) as c6, 
                                                    sum(cm.c7) as c7, sum(cm.c8) as c8 
                                                    from consolidado as f 
                                                    inner join consolidado_muestra as cm on (cm.idconsolidado=f.idconsolidado) 
                                                    inner join tipo_intervencion as t on (t.idtipointervencion=cm.idtipointervencion) 
                                                    ...
                                                    where  f.estareg=1 and f.fechainicio between '".$fechainicio."' and '".$fechafinal."' ".$and."
                                                    group by departamento,provincia,distrito,cm.localidad
